...
int main(int argc, char **argv) {
 ...
 try {
	...
	for(int i=0; i<vect.size(); i++){
	 ...
	 IloInt noGen, noDem; 
	 IloNum det_alpha;
	 IloNumArray cost(envData), us(envData), ccmin(envData), ccmax(envData), demand(envData), alpha(envData), det_demand(envData);
	 IloNumArray2 nom_prob(envData), f(envData);

///////////////// DATA FILE READING ////////////////////////////////

	  const char* filename  = "inputs1.dat";	      
	  ...
	  file >> ccmin >> ccmax >> cost >> f >> us >> nom_prob >> alpha >> demand >> det_alpha >> det_demand;

	  noGen = cost.getSize();
	  noDem = us.getSize();
	  
	  IloInt noStoch = 2;
	  IloIntArray noScen(envData, noStoch);
	  
	  for(i=0; i<noStoch; i++) {
		  noScen[i] = nom_prob[i].getSize(); 
	  }

	  IloBool consistentData = (ccmin.getSize() == noGen &&  alpha.getSize() == noScen[0] && demand.getSize() == noScen[1]);
	  ...
	  const IloInt numScen = noScen[0] * noScen[1];
	  IloNumArray prob(envData, numScen);


	  vector<array<int, 2>> realization;
	  w = 0;
	  for (w1 = 0; w1<noScen[0]; w1++) {
		  for (w2 = 0; w2<noScen[1]; w2++) {
			  prob[w] = nom_prob[0][w1] * nom_prob[1][w2];
			  array<int, 2> temp = { w1, w2 };
...
//////////BEGIN ITERATIONS/////////////////////////////////
	...
	Result <<"Iter"<<'\t'<<"Z_hat"<<'\t'<<"LB"<<'\t'<<"UB"<<'\t'<<"Relative_Gap"<<'\t'<<"Incumbent"<<endl;
		...
		while (rel_Gap > toler) {
			...
			for (w1 = 0; w1 < noScen[0]; w1++) {
				for (w2 = 0; w2 < noScen[1]; w2++) {
					...
					sub_obj += -x_hat[0] * Pi_Capacity[0] * alpha[w1] - x_hat[1] * Pi_Capacity[1] * det_alpha;

					sub_obj += Pi_Demand[0] * det_demand[0] + Pi_Demand[1] * demand[w2] + Pi_Demand[2] * det_demand[1];
					...
					IloExpr Gx(env);
					Gx -= X[0] * pi_capacity[w][0] * alpha[w1] + X[1] * pi_capacity[w][1] * det_alpha;

					Gx += pi_demand[w][0] * det_demand[0] + pi_demand[w][1] * demand[w2] + pi_demand[w][2] * det_demand[1];
		...
		for(w=0; w<numScen; w++){
			Result<< Output[w].No<< '\t';
			Result << "[";
			Result<<alpha[Output[w].Realization[0]] << ", ";
			Result << demand[Output[w].Realization[1]] << "]" << '\t';
			...
			if (Output[*it].nom_Prob<=rho){
			   ...
			   for(w=0; w<numScen; w++){
				...
				for (it=Unknownscen[ii].begin(); it!=Unknownscen[ii].end(); it++){
					Result<< Output[*it].No<< '\t';
					Result << "[";
					Result << alpha[Output[*it].Realization[0]] << ", ";
					Result << demand[Output[*it].Realization[1]] << "]" << '\t';
		...
		for (w=0; w<numScen; w++){
			...
			ScenarioRes<<alpha[realization[w][0]]<< '\t';
			ScenarioRes<<demand[realization[w][1]]<< '\t';
